...
@Service
@Slf4j
public class OrderFullfillmenntService {
	...
	public Order processOrder(Order order) throws InterruptedException {
		...
		if(inventoryService.checkProductAvailablity(order.getProductId())) {
	...
	public void assignTrailerAndDispatch(Order order) throws InterruptedException {
		Thread.sleep(3000L);
		log.info("Trailer assigned and dispatched " + Thread.currentThread().getName());
	}
